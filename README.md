...
The embeddings generated by our best performing models for MoleculeNet data can be directly downloaded here.
* TODO: add link to the pre-trained embeddings

Please place the downloaded "finetuning_datasets" folder in the __data__ directory if you want to perform SVM on the embeddings later. You can also generate embeddings for MoleculeNet data using the command below. Please note that "run_svm.py" script is mainly for testing performance of our pre-trained models on MoleculeNet data by performing SVM. If you want to use it only to generate embeddings, make sure to set the __--run_svm__ argument to 0.

```
python3 run_svm.py --dataset_path=data/finetuning_datasets --model_file=data/pretrained_models/modelO --generate_embeddings=1 --run_svm=0
```
