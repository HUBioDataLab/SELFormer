...
## Installation
SELFIES-Transformer is a command-line tool for pre-training and fine-tuning models using molecules represented by SELFIES notation. It should run in any Unix-like operating system. You can install it by first cloning rthe repository and then running commands below. [requirements.yml](/data/requirements.yml) is located under the data folder.
```
...
Please first download our pretrained models [here](https://drive.google.com/file/d/1zuVAKXCMc-HZHQo9y3Hu5zmQy51FGduI/view?usp=sharing)

Please run the command below to generate embeddings for an input dataset:
